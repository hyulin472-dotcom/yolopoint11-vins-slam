...
cv::Mat getColorImageFromMsg(const sensor_msgs::msg::Image::ConstPtr &img_msg)
{
    cv_bridge::CvImageConstPtr ptr;
    try
    {
    ...
    {
        cv::Mat gray = getImageFromMsg(img_msg);
        cv::Mat bgr;
        if (!gray.empty())
        ...
        if(STEREO)
        {
            ...
            if (!img0_buf.empty() && !img1_buf.empty())
            {
                ...
                else
                {
                    ...
                    color0 = getColorImageFromMsg(img0_buf.front());
                    img0_buf.pop();
            ...
            m_buf.unlock();
            if(!image0.empty())
                estimator.inputImage(time, image0, image1, color0);
        }
        else
        {
            cv::Mat image, color;
            ...
            if(!img0_buf.empty())
            {
                ...
                color = getColorImageFromMsg(img0_buf.front());
                img0_buf.pop();
                // Drop old buffered frames to keep up with real-time
            ...
            m_buf.unlock();
            if(!image.empty())
                estimator.inputImage(time, image, cv::Mat(), color);
        }
...
void imu_switch_callback(const std_msgs::msg::Bool::SharedPtr switch_msg)
{
    ...
    {
        //ROS_WARN("use IMU!");
        estimator.changeSensorType(1);
    }
    else
    {
        //ROS_WARN("disable IMU!");
        estimator.changeSensorType(0);
    }
    return;
}

void save_loop_callback(const std_msgs::msg::Bool::SharedPtr save_msg)
{
    if (!save_msg->data || !LOOP_CLOSURE_ENABLE)
        return;

    independentLoopClosure().waitUntilIdle();
    if (!independentLoopClosure().saveTrajectory(OUTPUT_FOLDER + "/loop_pose_graph.tum"))
        ROS_WARN("failed to save independent loop pose graph");
}
...
int main(int argc, char **argv)
{
    ...
    auto sub_img1 = n->create_subscription<sensor_msgs::msg::Image>(IMAGE1_TOPIC, rclcpp::SensorDataQoS(), img1_callback);
    ...
    auto sub_save_loop = n->create_subscription<std_msgs::msg::Bool>("/vins_save_loop", rclcpp::QoS(rclcpp::KeepLast(1)), save_loop_callback);
    ...
    if (LOOP_CLOSURE_ENABLE)
    {
        ...
        independentLoopClosure().saveOptimizedTrajectory(
            OUTPUT_FOLDER + "/loop_optimized.csv");
    }
