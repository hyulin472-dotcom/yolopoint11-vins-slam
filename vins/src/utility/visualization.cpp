...
OnlineDenseMapper online_dense_mapper;
...
void registerPub(rclcpp::Node::SharedPtr n)
{
    ...
    online_dense_mapper.init(n);
    independentLoopClosure().init(n);
...
{
    // Stop all producers before releasing their publishers.  These objects
    // otherwise have static storage duration and outlive the node/DDS
    // participant that created their DataWriters.
    independentLoopClosure().shutdown();
    online_dense_mapper.shutdown();
...
void resetOnlineDenseMap()
{
    online_dense_mapper.reset();
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, double t)
{
    ...
    independentLoopClosure().correctPose(P, Q, corrected_p, corrected_q);
    const Eigen::Vector3d corrected_v = independentLoopClosure().correctVector(V);
    odometry.pose.pose.position.x = corrected_p.x();
    odometry.pose.pose.position.y = corrected_p.y();
    odometry.pose.pose.position.z = corrected_p.z();
    odometry.pose.pose.orientation.x = corrected_q.x();
    odometry.pose.pose.orientation.y = corrected_q.y();
    odometry.pose.pose.orientation.z = corrected_q.z();
    odometry.pose.pose.orientation.w = corrected_q.w();
    odometry.twist.twist.linear.x = corrected_v.x();
    odometry.twist.twist.linear.y = corrected_v.y();
    odometry.twist.twist.linear.z = corrected_v.z();
...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        independentLoopClosure().recordOdometry(
            stamp, estimator.Ps[WINDOW_SIZE], tmp_Q, estimator.Vs[WINDOW_SIZE]);
        Vector3d corrected_p;
        Quaterniond corrected_q;
        independentLoopClosure().correctPose(estimator.Ps[WINDOW_SIZE], tmp_Q,
                                             corrected_p, corrected_q);
        const Vector3d corrected_v = independentLoopClosure().correctVector(estimator.Vs[WINDOW_SIZE]);
        odometry.pose.pose.position.x = corrected_p.x();
        odometry.pose.pose.position.y = corrected_p.y();
        odometry.pose.pose.position.z = corrected_p.z();
        odometry.pose.pose.orientation.x = corrected_q.x();
        ...
        foutC << corrected_p.x() << ","
              ...
              << corrected_v.z() << "," << endl;
        foutC.close();
        Eigen::Vector3d tmp_T = corrected_p;
        printf("time: %f, t: %f %f %f q: %f %f %f %f \n", header.stamp.sec + header.stamp.nanosec * (1e-9),
                                                          tmp_T.x(), tmp_T.y(), tmp_T.z(),
                                                          corrected_q.w(), corrected_q.x(), corrected_q.y(), corrected_q.z());
    }
}
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        std::vector<Eigen::Vector3d> loop_landmarks_world;
        std::vector<cv::Point2f> loop_landmark_pixels;
        std::vector<int> loop_landmark_ids;
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {
                ...
                const Vector2d &uv = it_per_id.feature_per_frame[imu_j].uv;
                if (w_pts_i.allFinite() && uv.allFinite())
                {
                    loop_landmarks_world.push_back(w_pts_i);
                    loop_landmark_pixels.emplace_back(static_cast<float>(uv.x()),
                                                       static_cast<float>(uv.y()));
                    loop_landmark_ids.push_back(it_per_id.feature_id);
                }
            }
        ...
        independentLoopClosure().submitKeyframe(estimator.Headers[i],
                                                estimator.dense_left_images[i],
                                                estimator.dense_right_images[i],
                                                estimator.Ps[i],
                                                estimator.Rs[i],
                                                loop_landmarks_world,
                                                loop_landmark_pixels,
                                                loop_landmark_ids,
                                                estimator.loop_model_features[i]);

        int dense_i = i;
        if (estimator.dense_left_images[dense_i].empty() || estimator.dense_right_images[dense_i].empty())
        {
            for (int j = i + 1; j <= WINDOW_SIZE; ++j)
            {
                if (!estimator.dense_left_images[j].empty() && !estimator.dense_right_images[j].empty())
                {
                    dense_i = j;
                    break;
                }
        ...
        std_msgs::msg::Header dense_header = odometry.header;
        int dense_sec_ts = static_cast<int>(estimator.Headers[dense_i]);
        uint dense_nsec_ts = static_cast<uint>((estimator.Headers[dense_i] - dense_sec_ts) * 1e9);
        dense_header.stamp.sec = dense_sec_ts;
        dense_header.stamp.nanosec = dense_nsec_ts;
        online_dense_mapper.processKeyframe(estimator.dense_left_images[dense_i],
                                            estimator.dense_right_images[dense_i],
                                            estimator.dense_left_color_images[dense_i],
                                            estimator.Ps[dense_i],
                                            estimator.Rs[dense_i],
                                            dense_header);
    }
}
